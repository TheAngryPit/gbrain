...
7. Connect an agent with [Connect an agent](#connect-an-agent) if you want one
   to use the brain.
8. Finish with the [minimum local verification](#verify).
...
gbrain doctor --json
gbrain search modes
mkdir -p ~/gbrain-notes
printf '# First note\n\nThe cedar lighthouse is the verification beacon for this guide.\n' \
  > ~/gbrain-notes/first-note.md
gbrain import ~/gbrain-notes --no-embed
...
gbrain init --mcp-only \
  ...
  --oauth-client-secret your_oauth_client_secret_here
gbrain doctor
gbrain remote ping
```

`gbrain remote ping` should confirm that the remote MCP endpoint is reachable.
If it fails, do not initialize a local brain as a workaround. Check the host URL,
client credentials, and server-side access with the host operator.
